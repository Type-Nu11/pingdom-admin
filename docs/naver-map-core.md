...
- `map.types.ts`의 MapHandle·MapMarker·MapProps는 공급자 독립 계약이다. 소비 화면 전환 후 #226에서 미사용 KakaoMap 및 호환 별칭을 제거했다.
- #223에서 관리자 화면, #224에서 상점주 등록 지도/주소, #225에서 서버 업체명 검색을 연결한다.
- 공통 기반 도입 당시에는 카카오 SDK·검색·데이터 필드·배포 정책을 유지했다. #226에서 SDK·설정만 정리했고 서버 데이터 필드는 유지한다.
...
- CSP는 Report-Only이며 #226에서 공개 SDK 소스에 근거해 네이버 출처를 반영하고 카카오 출처를 제거했다. 인증된 실제 SDK·배포 검증은 남아 있다. 상세 범위는 naver-map-cleanup.md를 참고하며 강제 정책 환경에서의 동작을 보장하지 않는다.
