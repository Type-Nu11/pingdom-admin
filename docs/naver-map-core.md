...
- 실제 SDK는 두 로컬 작업 폴더에 VITE_NAVER_MAP_CLIENT_ID가 없어 미수행. 실제 SDK·배포 도메인·타일·CSP 요청과 전체 화면 연계 검증은 #220·#221에 연결한다.
...
- 실제 네이버 SDK·인증·타일·운영 배포는 미검증이며 상점주 신규 등록과 관리자 화면의 실제 전환은 후속 이슈에서 진행한다.
