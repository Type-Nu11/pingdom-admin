...
- 신규 배열에 값이 있으면 우선한다. 서버는 구형 리뷰의 신규 컬렉션도 빈 배열로 반환하므로 빈 배열/누락/null이면 recommendReason/imageUrls로 대체한다. 신규/기존 값을 합치지 않는다.
- 신규 사진 배열이 채워져 있지만 URL/MIME 검증에 실패한 경우에는 기존 사진으로 우회하지 않는다. 기존 사진으로 대체할 때도 같은 URL 검증과 중복 제거를 적용한다.
- 추천 이유는 한국어로 표시하고 중복 제거한다. 미지원 enum은 기타 추천 이유로 표시한다. 구형 자유 입력은 보존한다.
- 작은 추천 이유 글자에는 primaryForeground를 사용한다. 브라우저에서 실제 렌더링 색상 대비가 흰 표면 기준 4.5:1 이상인지 검증한다.
...
- Chromium 공통 컴포넌트 fixture: 1280px/390px, 추천 이유 7개와 대비, 이미지 실패 및 리뷰 전환 복구, 신규 배열이 빈 구형 리뷰의 이유·사진 표시, 완전히 빈 리뷰 전환, 키보드 원본 열기, 가로 넘침 확인.
