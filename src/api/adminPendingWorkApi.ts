...
const checks: PendingWorkCheck[] = [
  ...
  async () => {
    const response = await getAdminReportAppeals({ status: 'SUBMITTED', page: 1, limit: 1 })
    return [item(
      'report-appeals',
      '신고 이의제기',
      '검토 대기 중인 신고 이의제기',
  ...
  async () => {
    const response = await getAdminMerchantOwners({ status: 'PENDING', page: 1, limit: 1 })
    return [item(
      'merchant-owner-applications',
      'Merchant Owner 신청',
      '승인 대기 중인 Merchant Owner 프로필',
      response.totalCount,
      '/merchant-owners',
    )]
  },
  ...
  async () => {
    const [reports, corrections] = await Promise.all([
      getAdminVisitorVerificationReports({ status: 'SUBMITTED', page: 1, limit: 1 }),
      getAdminVisitorVerificationCorrections({ status: 'SUBMITTED', page: 1, limit: 1 }),
    ])
    return [
      item(
  ...
  async () => {
    const response = await getAdminMerchantPlaceClaims({ status: 'PENDING', page: 1, limit: 1 })
    return [item(
