    ...
    <S.OperatingDialogOverlay role="presentation" onMouseDown={handleClose}>
      ...
      >
        <S.OperatingDialogHeader>
          <div>
            <S.OperatingDialogEyebrow>방문객 안내 정보</S.OperatingDialogEyebrow>
            <S.OperatingDialogTitle id="place-tourist-info-dialog-title">
              {place.name} 방문객 안내 정보 수정
            </S.OperatingDialogTitle>
          </div>
          <S.OperatingDialogCloseButton
            ref={closeButtonRef}
            type="button"
            aria-label="방문객 안내 정보 수정 닫기"
            disabled={isSaving}
            onClick={handleClose}
          >
        ...
        <S.OperatingDialogBody>
          <S.OperatingDialogDescription>
            앱에서 방문자에게 제공할 영문 이름과 안내 요약을 관리합니다. 비워서
            저장한 항목은 기존 값에서 제거됩니다.
          </S.OperatingDialogDescription>
          ...
          <S.OperatingFormField>
            ...
            <S.OperatingTextArea
              ...
              maxLength={500}
              placeholder="방문자가 장소를 이해하는 데 필요한 핵심 정보를 입력해주세요."
              disabled={isSaving}
              onChange={(event) => {
          ...
          <S.OperatingFormField>
            ...
            <S.OperatingTextArea
              value={reason}
              maxLength={500}
              placeholder="방문객 안내 정보를 수정하는 이유를 입력해주세요."
              disabled={isSaving}
              onChange={(event) => {
