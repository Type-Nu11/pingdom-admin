...
function Comments({ postId }: { postId: number }) {
  ...
  const load = useCallback((page: number, signal: AbortSignal) => getCommunityComments(postId, { page, hidden: hidden ? hidden === 'true' : undefined }, signal), [postId, hidden])
  const query = useCommunityListQuery(`comments:${postId}:${hidden}`, load)
  const { page } = query
  ...
  return <S.Body>
    ...
    <AdminSelect aria-label="댓글 노출 상태" value={hidden} onChange={e => { setHidden(e.target.value); setCommentId(null) }}>{hiddenOptions}</AdminSelect>
    ...
    <CommunityPagination data={query.pagination} page={page} loading={query.loading} onChange={p => { void query.changePage(p); setCommentId(null) }} label="댓글 페이지네이션" />
...
export default function CommunityContentPage() {
  ...
  const load = useCallback((page: number, signal: AbortSignal) => getCommunityPosts({ page, categoryId: filters.categoryId || undefined, hidden: filters.hidden ? filters.hidden === 'true' : undefined }, signal), [filters])
  const query = useCommunityListQuery(JSON.stringify(filters), load)
  const { page } = query
  ...
  return <CommunityShell title="커뮤니티 조회">
    ...
    <S.Filters onSubmit={e => { e.preventDefault(); setFilters({ categoryId: category.trim(), hidden }); setPostId(null) }}>
      ...
      <Shared.SecondaryButton type="button" onClick={() => { setCategory(''); setHidden(''); setFilters({ categoryId: '', hidden: '' }); setPostId(null) }}>초기화</Shared.SecondaryButton>
    ...
    <ListDetailWorkspace>
      <ListPane title="글 목록" count={query.data ? `${query.data.totalCount}건` : undefined} page={page} footer={<CommunityPagination data={query.pagination} page={page} loading={query.loading} label="글 페이지네이션" onChange={p => { void query.changePage(p); setPostId(null) }} />}>
