    ...
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}>
        ...
        <Shared.Content><Shared.PageStack>
          ...
          <S.MetricGrid><S.MetricCard><span>조회 범위</span><strong>페이지당 {limit.toLocaleString()}건</strong></S.MetricCard><S.MetricCard><span>미해결 이슈</span><strong>{total.toLocaleString()}건</strong></S.MetricCard><S.MetricCard><span>현재 페이지 오류</span><strong>{errorCount.toLocaleString()}건</strong></S.MetricCard><S.MetricCard><span>현재 결과</span><strong>{filteredIssues.length.toLocaleString()}건</strong></S.MetricCard></S.MetricGrid>
          <AdminStatusFilter label="이슈 필터" description="현재 페이지에 표시된 이슈를 심각도로 좁혀 봅니다." controls={<AdminStatusSelect aria-label="심각도 필터" value={severity} disabled={isLoading} onChange={(event) => setSeverity(event.target.value as DataQualityIssueSeverity | '')}><option value="">심각도 전체</option>{Object.entries(SEVERITY_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</AdminStatusSelect>} />
          <Shared.Workspace>
            <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>감지 이슈</Shared.PanelTitle><Shared.PanelDescription>미해결 이슈를 선택해 규칙과 감지 사유를 확인합니다.</Shared.PanelDescription></div><Shared.PanelCount>{total.toLocaleString()}건</Shared.PanelCount></Shared.PanelHeader><Shared.ScrollArea>{isLoading && issues.length === 0 ? <Shared.EmptyState><strong>데이터 품질 이슈를 불러오는 중입니다.</strong></Shared.EmptyState> : filteredIssues.length === 0 ? <Shared.EmptyState><strong>조건에 맞는 이슈가 없습니다.</strong></Shared.EmptyState> : <S.CardList>{filteredIssues.map((issue) => <S.RecordButton key={issueKey(issue)} type="button" $selected={selectedKey === issueKey(issue)} onClick={() => setSelectedKey(issueKey(issue))}><S.RecordHeader><S.RecordTitle>{entityLabel(issue.entityType)} #{issue.entityId}</S.RecordTitle><S.StatusBadge $tone={severityTone(issue.severity)}>{SEVERITY_LABELS[issue.severity]}</S.StatusBadge></S.RecordHeader><S.RecordMeta>{issue.ruleCode} · 감지 {formatDate(issue.detectedAt)}</S.RecordMeta><S.RecordDescription>{issue.details || '상세 사유가 제공되지 않았습니다.'}</S.RecordDescription></S.RecordButton>)}</S.CardList>}</Shared.ScrollArea>{totalPages > 1 ? <S.Pagination><Shared.SecondaryButton type="button" disabled={isLoading || page <= 1} onClick={() => void fetchIssues(page - 1)}>이전</Shared.SecondaryButton><span>{page} / {totalPages}</span><Shared.SecondaryButton type="button" disabled={isLoading || !hasNext} onClick={() => void fetchIssues(page + 1)}>다음</Shared.SecondaryButton></S.Pagination> : null}</Shared.Panel>
