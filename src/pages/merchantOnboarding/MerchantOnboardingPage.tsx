...
function MerchantOnboardingPage() {
  ...
  if (onboarding.status === 'error') {
    return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>상점주 신청</Store.PageTitle></div></Store.PageIntro><Store.Notice $tone="error" role="alert"><Store.NoticeIcon aria-hidden="true">error_outline</Store.NoticeIcon>{onboarding.errorMessage}</Store.Notice><div style={{ marginTop: 16 }}><Store.RetryButton type="button" onClick={() => void onboarding.fetchOnboarding()}>다시 시도</Store.RetryButton></div></Store.Content></Store.Page>
  }
  ...
  return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.HeaderUser><Store.AccountIcon aria-hidden="true">storefront</Store.AccountIcon><strong>{user?.username ?? '사용자'}</strong><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.HeaderUser></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>상점주 신청</Store.PageTitle><Store.PageDescription>검증이 완료되면 내 가게의 정보와 운영 기능을 관리할 수 있습니다.</Store.PageDescription></div></Store.PageIntro>
    <S.Stack>
      ...
      <S.Panel>
        ...
        {stage === 'profile' ? <><S.IntroNotice><strong>검증이 필요한 이유</strong><span>검증된 상점주만 장소 운영, 예약, 혜택, 정산 기능을 사용할 수 있습니다.</span></S.IntroNotice><ProfileForm profile={null} isSaving={onboarding.isSavingProfile} isReapplying={false} onSave={onboarding.saveProfile} /></> : null}
