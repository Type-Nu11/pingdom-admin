...
function MerchantOwnerPage() {
  ...
  const submit = async () => {
    ...
    if (dialog.type === 'review') {
      const result = await hook.review(userId, { reason: trimmedReason })
    ...
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}><Shell.TopBar><Shell.TopTitleGroup><Shell.TopTitle>상점주 관리</Shell.TopTitle></Shell.TopTitleGroup><Shell.TopActions><AdminNotificationButton /><Shell.IconButton type="button" aria-label="목록 새로고침" disabled={hook.isLoading || hook.activeAction !== null} onClick={() => void hook.fetchProfiles(hook.status, hook.page)}><Shell.MaterialIcon aria-hidden="true">refresh</Shell.MaterialIcon></Shell.IconButton></Shell.TopActions></Shell.TopBar>
        <Shared.Content><Shared.PageStack>
          <Shared.PageHeader><div><Shared.Eyebrow>성장 운영 &gt; 상점주 관리</Shared.Eyebrow><Shared.PageTitle>상점주 관리</Shared.PageTitle><Shared.PageDescription>승인된 상점주의 연결 장소, 온보딩, 운영 품질을 관리합니다.</Shared.PageDescription></div><Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-place-applications')}>상점주 장소 신청 심사</Shared.HeaderButton></Shared.HeaderActions></Shared.PageHeader>
          ...
          <AdminStatusFilter label="상점주 상태" description="운영 중이거나 권한이 회수된 상점주를 조회합니다." value={hook.status} disabled={hook.isLoading || hook.activeAction !== null} onChange={(event) => changeStatus(event.target.value as MerchantOwnerStatus | '')}><option value="ACTIVE">운영 중</option><option value="REVOKED">권한 회수</option></AdminStatusFilter>
