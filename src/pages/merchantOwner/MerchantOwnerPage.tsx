...
function MerchantOwnerPage() {
  ...
  return (
    <Shell.AppShell>
      <Shell.SideNav aria-label="관리자 메뉴"><Shell.SideHeader><Shell.BrandLockup><Shell.BrandLogo src="/pingdom-logo.png" alt="PingDom" /></Shell.BrandLockup></Shell.SideHeader><Shell.SideMenu><Shell.MenuButton type="button" $active onClick={() => navigate('/dashboard')}><Shell.MaterialIcon aria-hidden="true">dashboard</Shell.MaterialIcon><span>대시보드</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/places')}><Shell.MaterialIcon aria-hidden="true">location_on</Shell.MaterialIcon><span>장소 관리</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/main')}><Shell.MaterialIcon aria-hidden="true">description</Shell.MaterialIcon><span>게시글 관리</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/bans')}><Shell.MaterialIcon aria-hidden="true">block</Shell.MaterialIcon><span>사용자 밴</span></Shell.MenuButton><AdminNavigationMenu /></Shell.SideMenu><Shell.SideFooter><Shell.AdminProfile aria-label="관리자 계정"><Shell.AdminProfileIcon><Shell.MaterialIcon aria-hidden="true">admin_panel_settings</Shell.MaterialIcon></Shell.AdminProfileIcon><Shell.AdminProfileText><strong>{adminIdentifier}</strong><span>관리자</span></Shell.AdminProfileText></Shell.AdminProfile><Shell.LogoutButton type="button" onClick={() => { void logout(); navigate('/login', { replace: true }) }}><Shell.MaterialIcon aria-hidden="true">logout</Shell.MaterialIcon><span>로그아웃</span></Shell.LogoutButton></Shell.SideFooter></Shell.SideNav>
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}><Shell.TopBar><Shell.TopTitleGroup><Shell.TopTitle>Merchant Owner 운영</Shell.TopTitle></Shell.TopTitleGroup><Shell.TopActions><AdminNotificationButton /><Shell.IconButton type="button" aria-label="목록 새로고침" disabled={hook.isLoading || hook.activeAction !== null} onClick={() => void hook.fetchProfiles(hook.status, hook.page)}><Shell.MaterialIcon aria-hidden="true">refresh</Shell.MaterialIcon></Shell.IconButton></Shell.TopActions></Shell.TopBar>
        <Shared.Content><Shared.PageStack>
          <Shared.PageHeader><div><Shared.Eyebrow>대시보드 &gt; Merchant Owner</Shared.Eyebrow><Shared.PageTitle>Merchant Owner 신청 및 운영</Shared.PageTitle><Shared.PageDescription>신청 심사부터 온보딩, 연결 장소, 운영 품질까지 사용자 단위로 관리합니다.</Shared.PageDescription></div><Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-place-claims')}>장소 Claim 심사</Shared.HeaderButton><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-verifications')}>검증 심사</Shared.HeaderButton><Shared.HeaderButton type="button" onClick={() => navigate('/dashboard')}>대시보드</Shared.HeaderButton></Shared.HeaderActions></Shared.PageHeader>
          ...
          <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>신청 상태</Shared.PanelTitle><Shared.PanelDescription>상태별 Merchant Owner 프로필을 조회합니다.</Shared.PanelDescription></div><S.Select value={hook.status} disabled={hook.isLoading || hook.activeAction !== null} onChange={(event) => changeStatus(event.target.value as MerchantOwnerStatus | '')}><option value="">전체</option>{Object.entries(OWNER_STATUS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</S.Select></Shared.PanelHeader></Shared.Panel>
