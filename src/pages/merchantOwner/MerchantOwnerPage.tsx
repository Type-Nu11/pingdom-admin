...
function MerchantOwnerPage() {
  ...
  return (
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}><Shell.TopBar><Shell.TopTitleGroup><Shell.TopTitle>상점주 신청 및 운영</Shell.TopTitle></Shell.TopTitleGroup><Shell.TopActions><AdminNotificationButton /><Shell.IconButton type="button" aria-label="목록 새로고침" disabled={hook.isLoading || hook.activeAction !== null} onClick={() => void hook.fetchProfiles(hook.status, hook.page)}><Shell.MaterialIcon aria-hidden="true">refresh</Shell.MaterialIcon></Shell.IconButton></Shell.TopActions></Shell.TopBar>
        <Shared.Content><Shared.PageStack>
          <Shared.PageHeader><div><Shared.Eyebrow>검토함 &gt; 상점주 신청</Shared.Eyebrow><Shared.PageTitle>상점주 신청 및 운영</Shared.PageTitle><Shared.PageDescription>신청 심사부터 온보딩, 연결 장소, 운영 품질까지 사용자 단위로 관리합니다.</Shared.PageDescription></div><Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-place-applications')}>장소 운영 신청 심사</Shared.HeaderButton><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-place-claims')}>기존 장소 운영 신청 심사</Shared.HeaderButton><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-verifications')}>상점주 검증 심사</Shared.HeaderButton></Shared.HeaderActions></Shared.PageHeader>
          ...
          <AdminStatusFilter label="신청 상태" description="상태별 상점주 신청 정보를 조회합니다." value={hook.status} disabled={hook.isLoading || hook.activeAction !== null} onChange={(event) => changeStatus(event.target.value as MerchantOwnerStatus | '')}><option value="">전체</option>{Object.entries(OWNER_STATUS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</AdminStatusFilter>
          ...
          <Shared.Workspace>
            ...
            <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>신청 정보 및 연결 장소</Shared.PanelTitle><Shared.PanelDescription>상태 변경 전 연락처와 장소 운영 품질을 확인합니다.</Shared.PanelDescription></div></Shared.PanelHeader><Shared.CompareBody>{!selectedUserId ? <Shared.EmptyState><strong>확인할 상점주 신청을 선택해주세요.</strong></Shared.EmptyState> : hook.isDetailLoading ? <Shared.EmptyState><strong>상세 정보를 불러오는 중입니다.</strong></Shared.EmptyState> : hook.detailErrorMessage ? <Shared.EmptyState><strong>{hook.detailErrorMessage}</strong><Shared.SecondaryButton type="button" onClick={() => void hook.fetchDetail(selectedUserId)}>다시 시도</Shared.SecondaryButton></Shared.EmptyState> : hook.profile ? <>
