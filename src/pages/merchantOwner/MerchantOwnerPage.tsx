...
import { ADMIN_OWNER_PAGE_SIZE, useAdminMerchantOwners } from '../../hooks/useAdminMerchantOwners'
...
function MerchantOwnerPage() {
  ...
  const openDialog = (next: Dialog) => {
    ...
    setReason(''); setFormError(''); hook.dismissActionError()
    ...
    if (dialog.type === 'review') {
      ...
      const result = dialog.action === 'approve'
        ...
        : dialog.action === 'reject'
          ? await hook.reject(userId, request)
          : await hook.revoke(userId, request)
  ...
  return (
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}><Shell.TopBar><Shell.TopTitleGroup><Shell.TopTitle>상점주 관리</Shell.TopTitle></Shell.TopTitleGroup><Shell.TopActions><AdminNotificationButton /><Shell.IconButton type="button" aria-label="목록 새로고침" disabled={hook.isLoading || hook.activeAction !== null} onClick={() => void hook.fetchProfiles(hook.status, hook.page)}><Shell.MaterialIcon aria-hidden="true">refresh</Shell.MaterialIcon></Shell.IconButton></Shell.TopActions></Shell.TopBar>
        <ListDetailPage>
          <Shared.PageHeader><Shared.PageTitle>상점주 심사·운영 관리</Shared.PageTitle><Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-place-applications')}>상점주 장소 신청 심사</Shared.HeaderButton></Shared.HeaderActions></Shared.PageHeader>
          {hook.actionErrorMessage ? <FeedbackMessage tone="error" onDismiss={hook.dismissActionError}>{hook.actionErrorMessage}</FeedbackMessage> : null}{hook.successMessage ? <Shared.Notice $variant="success" role="status">{hook.successMessage}</Shared.Notice> : null}
          <AdminStatusFilter label="상점주 상태" description="기본 상점주 신청과 운영 상태를 구분해 조회합니다. 장소 신청 심사는 별도 화면에서 처리합니다." value={hook.status} disabled={hook.isLoading || hook.activeAction !== null} onChange={(event) => changeStatus(event.target.value as MerchantOwnerStatus | '')}><option value="PENDING">심사 대기</option><option value="ACTIVE">운영 중</option><option value="REJECTED">반려</option><option value="REVOKED">권한 회수</option></AdminStatusFilter>
          {hook.errorMessage ? <Shared.Notice $variant="error" role="alert">{hook.errorMessage}</Shared.Notice> : null}
          <ListDetailWorkspace>
            <ListPane title={`${hook.status ? OWNER_STATUS[hook.status] : '상점주'} 목록`} range={!hook.isLoading && !hook.errorMessage ? { page: hook.page, pageSize: ADMIN_OWNER_PAGE_SIZE, itemCount: hook.profiles.length, total: hook.totalCount } : undefined} page={hook.page} ariaLabel="상점주 목록" footer={hook.totalPages > 1 ? <AdminPagination ariaLabel="상점주 목록 페이지네이션" page={hook.page} totalPages={hook.totalPages} hasNext={hook.hasNext} disabled={hook.isLoading} onPageChange={(nextPage) => { setSelectedUserId(null); hook.clearDetail(); void hook.fetchProfiles(hook.status, nextPage) }} /> : null}>{hook.isLoading && hook.profiles.length === 0 ? <Shared.EmptyState><strong>상점주 목록을 불러오는 중입니다.</strong></Shared.EmptyState> : hook.profiles.length === 0 ? <Shared.EmptyState><strong>조건에 맞는 상점주가 없습니다.</strong></Shared.EmptyState> : <S.CardList>{hook.profiles.map((profile) => <S.RecordButton key={profile.userId} type="button" $selected={selectedUserId === profile.userId} onClick={() => selectProfile(profile.userId)}><S.RecordHeader><S.RecordTitle>{profile.businessName}</S.RecordTitle><S.StatusBadge $tone={profile.status === 'ACTIVE' ? 'success' : profile.status === 'PENDING' ? 'warning' : 'danger'}>{OWNER_STATUS[profile.status]}</S.StatusBadge></S.RecordHeader><S.RecordMeta>{profile.displayName} · 사용자 #{profile.userId}</S.RecordMeta><S.RecordSummary>{profile.description || '사업자 소개 없음'}</S.RecordSummary></S.RecordButton>)}</S.CardList>}</ListPane>
      ...
      {dialog && hook.profile ? <Shared.ModalOverlay role="presentation" onMouseDown={() => hook.activeAction === null && setDialog(null)}><Shared.Modal role="dialog" aria-modal="true" aria-labelledby="merchant-owner-dialog-title" onMouseDown={(event) => event.stopPropagation()}><Shared.ModalHeader><Shared.ModalTitle id="merchant-owner-dialog-title">{dialog.type === 'review' ? dialog.action === 'approve' ? '상점주 신청 승인' : dialog.action === 'reject' ? '상점주 신청 반려' : '상점주 권한 회수' : dialog.type === 'places' ? '연결 장소 변경' : dialog.type === 'onboarding' ? '온보딩 변경' : `장소 #${dialog.place.placeId} 운영 품질`}</Shared.ModalTitle><Shared.ModalCloseButton type="button" aria-label="닫기" disabled={hook.activeAction !== null} onClick={() => setDialog(null)}><Shell.MaterialIcon aria-hidden="true">close</Shell.MaterialIcon></Shared.ModalCloseButton></Shared.ModalHeader><Shared.ModalBody><S.FormGrid>
        {dialog.type === 'places' ? <S.WideField>연결 장소 ID<S.Input value={placeIds} placeholder="101, 102, 103" disabled={hook.activeAction !== null} onChange={(event) => { setPlaceIds(event.target.value); setFormError(''); hook.dismissActionError() }} /><small>쉼표로 구분하며, 비우면 연결 장소가 없는 상태입니다.</small></S.WideField> : null}
        ...
        <S.WideField>처리 사유 *<S.TextArea value={reason} maxLength={500} disabled={hook.activeAction !== null} onChange={(event) => { setReason(event.target.value); setFormError(''); hook.dismissActionError() }} /><small>{reason.length}/500</small></S.WideField>
      </S.FormGrid>{formError || hook.actionErrorMessage ? <FeedbackMessage tone="error" onDismiss={() => { setFormError(''); hook.dismissActionError() }}>{formError || hook.actionErrorMessage}</FeedbackMessage> : null}</Shared.ModalBody><Shared.ModalFooter><Shared.SecondaryButton type="button" disabled={hook.activeAction !== null} onClick={() => setDialog(null)}>취소</Shared.SecondaryButton><Shared.PrimaryButton type="button" disabled={hook.activeAction !== null} onClick={() => void submit()}>{hook.activeAction ? '처리 중' : dialog.type === 'review' ? dialog.action === 'approve' ? '승인 확정' : dialog.action === 'reject' ? '반려 확정' : '권한 회수 확정' : '변경 확정'}</Shared.PrimaryButton></Shared.ModalFooter></Shared.Modal></Shared.ModalOverlay> : null}
    </Shell.AppShell>
  )
}
