  ...
  return (
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}>
        <Shell.TopBar>
          <Shell.TopTitleGroup><Shell.TopTitle>장소 Claim 심사</Shell.TopTitle></Shell.TopTitleGroup>
        ...
        <Shared.Content><Shared.PageStack>
          <Shared.PageHeader>
            <div><Shared.Eyebrow>Merchant 운영 &gt; 장소 Claim</Shared.Eyebrow><Shared.PageTitle>Merchant 장소 Claim 심사</Shared.PageTitle><Shared.PageDescription>장소 운영 권한 또는 소유권 이전 요청을 장소·신청자·증빙 기준으로 검토합니다.</Shared.PageDescription></div>
            <Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-owners')}>Merchant Owner 운영</Shared.HeaderButton></Shared.HeaderActions>
          </Shared.PageHeader>
          ...
          <Shared.Workspace>
            <Shared.Panel>
              <Shared.PanelHeader><div><Shared.PanelTitle>Claim 요청</Shared.PanelTitle></div><Shared.PanelCount>{hook.total.toLocaleString()}건</Shared.PanelCount></Shared.PanelHeader>
              <Shared.ScrollArea>{hook.claims.length === 0 ? <Shared.EmptyState><strong>{hook.isLoading ? '불러오는 중입니다.' : '요청이 없습니다.'}</strong></Shared.EmptyState> : <S.CardList>{hook.claims.map((item) => <S.RecordButton key={item.id} type="button" $selected={hook.detail?.id === item.id} onClick={() => selectClaim(item.id)}><S.RecordHeader><S.RecordTitle>Claim #{item.id} · 장소 #{item.placeId}</S.RecordTitle><S.StatusBadge $tone={statusTone(item.status)}>{STATUS[item.status]}</S.StatusBadge></S.RecordHeader><S.RecordMeta>신청자 #{item.merchantOwnerUserId} · {TYPES[item.claimType]}</S.RecordMeta></S.RecordButton>)}</S.CardList>}</Shared.ScrollArea>
            ...
            <Shared.Panel>
              <Shared.PanelHeader><div><Shared.PanelTitle>Claim 상세</Shared.PanelTitle></div></Shared.PanelHeader>
              <Shared.CompareBody>{hook.isDetailLoading ? <Shared.EmptyState><strong>상세 조회 중입니다.</strong></Shared.EmptyState> : !hook.detail ? <Shared.EmptyState><strong>Claim 요청을 선택해주세요.</strong></Shared.EmptyState> : <><S.RecordHeader><div><S.RecordTitle>{hook.detail.place?.name || `장소 #${hook.detail.placeId}`}</S.RecordTitle><S.RecordMeta>신청자 #{hook.detail.merchantOwnerUserId} · {TYPES[hook.detail.claimType]}</S.RecordMeta></div><S.StatusBadge $tone={statusTone(hook.detail.status)}>{STATUS[hook.detail.status]}</S.StatusBadge></S.RecordHeader><S.RecordDescription>{hook.detail.claimReason}</S.RecordDescription><S.DetailGrid><S.DetailItem><dt>주소</dt><dd>{hook.detail.place?.address || '정보 없음'}</dd></S.DetailItem><S.DetailItem><dt>카테고리</dt><dd>{hook.detail.place?.category || '정보 없음'}</dd></S.DetailItem><S.DetailItem><dt>이전 소유자</dt><dd>{hook.detail.previousOwnerUserId ? `#${hook.detail.previousOwnerUserId}` : '해당 없음'}</dd></S.DetailItem><S.DetailItem><dt>신청 시각</dt><dd>{date(hook.detail.createdAt)}</dd></S.DetailItem></S.DetailGrid><ClaimAttachments /></>}</Shared.CompareBody>
            </Shared.Panel>
          </Shared.Workspace>
        </Shared.PageStack></Shared.Content>
      </Shell.MainArea>
      {decision !== null && hook.detail ? <Shared.ModalOverlay role="presentation" onMouseDown={() => !hook.isReviewing && setDecision(null)}><Shared.Modal role="dialog" aria-modal="true" aria-labelledby="claim-review-title" onMouseDown={(event) => event.stopPropagation()}><Shared.ModalHeader><Shared.ModalTitle id="claim-review-title">Claim 요청 {decision ? '승인' : '거절'}</Shared.ModalTitle><Shared.ModalCloseButton type="button" aria-label="닫기" disabled={hook.isReviewing} onClick={() => setDecision(null)}><Shell.MaterialIcon aria-hidden="true">close</Shell.MaterialIcon></Shared.ModalCloseButton></Shared.ModalHeader><Shared.ModalBody><Shared.ModalWarning>Claim #{hook.detail.id}의 장소 운영 권한이 변경될 수 있습니다. 상세 version {hook.detail.version} 기준으로 처리합니다.</Shared.ModalWarning><S.Section><S.Field>심사 사유 *<S.TextArea value={reason} maxLength={500} disabled={hook.isReviewing} onChange={(event) => { setReason(event.target.value); setFormError('') }} /></S.Field></S.Section>{formError || hook.actionErrorMessage ? <Shared.Notice $variant="error">{formError || hook.actionErrorMessage}</Shared.Notice> : null}</Shared.ModalBody><Shared.ModalFooter><Shared.SecondaryButton type="button" disabled={hook.isReviewing} onClick={() => setDecision(null)}>취소</Shared.SecondaryButton><Shared.PrimaryButton type="button" disabled={hook.isReviewing} onClick={() => void submit()}>{hook.isReviewing ? '처리 중' : '심사 확정'}</Shared.PrimaryButton></Shared.ModalFooter></Shared.Modal></Shared.ModalOverlay> : null}
    </Shell.AppShell>
  )

  function ClaimAttachments() {
    if (!hook.detail) return null
    return <><S.Section><S.SectionTitle>Claim 제출 증빙</S.SectionTitle>{hook.isAttachmentLoading ? <S.RecordDescription>증빙 파일 목록을 불러오는 중입니다.</S.RecordDescription> : hook.attachmentErrorMessage ? <><Shared.Notice $variant="error">{hook.attachmentErrorMessage}</Shared.Notice><S.InlineActions><Shared.SecondaryButton type="button" onClick={() => void hook.fetchAttachments(hook.detail!.id)}>다시 시도</Shared.SecondaryButton></S.InlineActions></> : hook.attachments.length === 0 ? <S.RecordDescription>제출된 Claim 증빙 파일이 없습니다.</S.RecordDescription> : <Attach.AttachmentList>{hook.attachments.map((attachment) => <Attach.AttachmentRow key={attachment.id}><div><strong>{DOCUMENTS[attachment.documentType]} · 파일 #{attachment.id}</strong><span>{attachment.contentType || '파일'} · {size(attachment.fileSize)} · 순서 {attachment.displayOrder + 1}</span></div><Attach.AttachmentButton type="button" disabled={hook.downloadingAttachmentId !== null} onClick={() => void hook.downloadAttachment(hook.detail!.id, attachment)}><Shell.MaterialIcon aria-hidden="true">download</Shell.MaterialIcon>{hook.downloadingAttachmentId === attachment.id ? '다운로드 중' : '다운로드'}</Attach.AttachmentButton></Attach.AttachmentRow>)}</Attach.AttachmentList>}</S.Section>{hook.detail.reviewReason ? <S.RecordDescription>심사 사유: {hook.detail.reviewReason}</S.RecordDescription> : null}{hook.detail.status === 'PENDING' ? <S.InlineActions><Shared.SecondaryButton type="button" onClick={() => { setDecision(false); setReason(''); setFormError('') }}>거절</Shared.SecondaryButton><Shared.PrimaryButton type="button" onClick={() => { setDecision(true); setReason(''); setFormError('') }}>승인</Shared.PrimaryButton></S.InlineActions> : null}</>
  }
}
