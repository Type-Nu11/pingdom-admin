...
const STATUS: Record<MerchantPlaceClaimStatus, { label: string; tone: 'draft' | 'published' | 'closed' }> = {
  ...
  APPROVED: { label: '승인', tone: 'published' },
...
function MerchantPlaceClaimRequestPage() {
  ...
  if (claims.status === 'error') {
    return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>장소 권한·소유권 신청</Store.PageTitle></div></Store.PageIntro><Store.Notice $tone="error" role="alert"><Store.NoticeIcon aria-hidden="true">error_outline</Store.NoticeIcon>{claims.errorMessage}</Store.Notice><div style={{ marginTop: 16 }}><Store.RetryButton type="button" onClick={() => void claims.fetchClaims(1, true)}>다시 시도</Store.RetryButton></div></Store.Content></Store.Page>
  }
  ...
  return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.HeaderUser><Store.AccountIcon aria-hidden="true">storefront</Store.AccountIcon><strong>{user?.username || '상점주'}</strong><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.HeaderUser></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>장소 권한·소유권 신청</Store.PageTitle><Store.PageDescription>PingDom에 등록된 장소의 운영 권한 또는 소유권 변경을 신청하고 증빙 파일과 심사 결과를 관리합니다.</Store.PageDescription></div><Shared.HeaderActions><Shared.HeaderButton type="button" disabled={claims.isLoading || claims.activeAction !== null} onClick={() => void claims.fetchClaims(claims.pageInfo.page)}>새로고침</Shared.HeaderButton></Shared.HeaderActions></Store.PageIntro>
