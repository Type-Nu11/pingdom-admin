...
function MerchantPlaceOperationsPage() {
  ...
  if (operations.status === 'error') {
    return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>장소 운영 정보</Store.PageTitle></div></Store.PageIntro><Store.Notice $tone="error" role="alert"><Store.NoticeIcon aria-hidden="true">error_outline</Store.NoticeIcon>{operations.errorMessage}</Store.Notice><div style={{ marginTop: 16 }}><Store.RetryButton type="button" onClick={() => void operations.fetchInitialData()}>다시 시도</Store.RetryButton></div></Store.Content></Store.Page>
  }
  ...
  return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.HeaderUser><Store.AccountIcon aria-hidden="true">storefront</Store.AccountIcon><strong>{operations.profile?.displayName || user?.username || '상점주'}</strong><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.HeaderUser></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>장소 운영 정보</Store.PageTitle><Store.PageDescription>방문자에게 보이는 영업 상태, 영업시간과 탐색 이미지를 최신 정보로 관리합니다.</Store.PageDescription></div><S.HeaderActions><S.HeaderButton type="button" disabled={operations.status === 'loading' || isBusy || !operations.selectedPlaceId} onClick={() => operations.selectedPlaceId && void operations.fetchPlaceOperations(operations.selectedPlaceId)}>새로고침</S.HeaderButton></S.HeaderActions></Store.PageIntro>
    ...
    {operations.status === 'loading' || operations.isLoading ? <Store.LoadingSummary aria-label="장소 운영 정보를 불러오는 중"><Store.Skeleton $height={312} /><Store.Skeleton $height={560} /></Store.LoadingSummary> : !operations.selectedPlaceId ? <Store.EmptyStoreState><Store.EmptyStoreIcon aria-hidden="true">add_business</Store.EmptyStoreIcon><div><Store.EmptyStoreTitle>관리할 장소가 아직 없습니다.</Store.EmptyStoreTitle><Store.EmptyStoreDescription>운영할 장소를 신청하거나 새 장소를 등록한 뒤, 승인되면 영업 상태와 탐색 미디어를 관리할 수 있습니다.</Store.EmptyStoreDescription></div><Store.EmptyStoreActions><Store.EmptyStoreAction type="button" onClick={() => navigate('/merchant/place-application')}>기존 장소 신청</Store.EmptyStoreAction><Store.EmptyStoreSecondaryAction type="button" onClick={() => navigate('/merchant/place-registration')}>새 장소 등록</Store.EmptyStoreSecondaryAction></Store.EmptyStoreActions></Store.EmptyStoreState> : <S.Workspace><S.Column><S.Panel><S.PanelHeader><div><S.PanelTitle>운영 상태</S.PanelTitle><S.PanelDescription>임시 휴업이나 폐업 상태를 즉시 반영합니다.</S.PanelDescription></div></S.PanelHeader><S.PanelBody><S.StatusOptions>{STATUS_OPTIONS.map((option) => <S.StatusOption key={option.value} $selected={nextStatus === option.value} $danger={option.danger}><input type="radio" name="merchant-place-status" value={option.value} checked={nextStatus === option.value} disabled={isBusy} onChange={() => setSelectedStatus(option.value)} /><div><strong>{option.label}</strong><small>{option.description}</small></div></S.StatusOption>)}</S.StatusOptions>{nextStatus === 'PERMANENTLY_CLOSED' ? <S.DangerNotice>폐업으로 변경하면 해당 장소의 운영 상태가 방문자 화면에 즉시 반영됩니다.</S.DangerNotice> : null}<S.FormActions><S.ActionButton type="button" $variant={nextStatus === 'PERMANENTLY_CLOSED' ? 'danger' : 'primary'} disabled={isBusy || nextStatus === currentStatus} onClick={() => void saveStatus()}>{operations.activeAction === 'status' ? '저장 중' : '상태 저장'}</S.ActionButton></S.FormActions></S.PanelBody></S.Panel><S.Panel><S.PanelHeader><div><S.PanelTitle>탐색 미디어</S.PanelTitle><S.PanelDescription>대표 이미지와 등록된 미디어를 확인합니다.</S.PanelDescription></div></S.PanelHeader><S.PanelBody><S.UploadArea><span>이미지 파일을 업로드하면 탐색 미디어로 바로 등록됩니다. 최대 10MB</span><input ref={mediaInputRef} type="file" accept="image/*" hidden onChange={handleMediaFileChange} /><S.UploadButton type="button" disabled={isBusy} onClick={() => mediaInputRef.current?.click()}>{operations.activeAction === 'upload-media' ? '업로드 중' : '이미지 업로드'}</S.UploadButton></S.UploadArea>{mediaItems.length > 0 ? <S.MediaGrid>{mediaItems.map((media, index) => <S.MediaItem key={media.id} $representative={representativeMediaId === media.id}><S.MediaImage src={media.thumbnailUrl || media.imageUrl} alt={`탐색 미디어 ${index + 1}`} /><S.MediaBody><S.MediaMeta>{representativeMediaId === media.id ? <S.RepresentativeBadge>대표 이미지</S.RepresentativeBadge> : <span>탐색 이미지</span>}<span>노출 순서 {index + 1}</span></S.MediaMeta><S.MediaActions><S.MediaOrderActions><S.RepresentativeMediaButton type="button" $selected={representativeMediaId === media.id} aria-label={representativeMediaId === media.id ? '현재 대표 이미지' : '대표 이미지로 설정'} title={representativeMediaId === media.id ? '현재 대표 이미지' : '대표 이미지로 설정'} disabled={isBusy || representativeMediaId === media.id} onClick={() => void operations.updateRepresentativeMedia(media.id)}>{representativeMediaId === media.id ? 'star' : 'star_outline'}</S.RepresentativeMediaButton><S.IconButton type="button" aria-label="노출 순서 한 단계 앞으로" disabled={isBusy || index === 0} title="노출 순서 한 단계 앞으로" onClick={() => void operations.moveMedia(media.id, 'previous')}>arrow_upward</S.IconButton><S.IconButton type="button" aria-label="노출 순서 한 단계 뒤로" disabled={isBusy || index === mediaItems.length - 1} title="노출 순서 한 단계 뒤로" onClick={() => void operations.moveMedia(media.id, 'next')}>arrow_downward</S.IconButton><S.IconButton type="button" $danger aria-label="미디어 삭제" title="미디어 삭제" disabled={isBusy} onClick={() => setDialog({ kind: 'delete-media', media })}>delete</S.IconButton></S.MediaOrderActions></S.MediaActions></S.MediaBody></S.MediaItem>)}</S.MediaGrid> : <S.Empty>등록된 탐색 미디어가 없습니다.</S.Empty>}</S.PanelBody></S.Panel></S.Column><S.Column><S.Panel><S.PanelHeader><div><S.PanelTitle>영업시간</S.PanelTitle><S.PanelDescription>정규 영업시간과 특정 날짜의 휴무·대체 영업시간을 설정합니다.</S.PanelDescription></div></S.PanelHeader>{operations.operating ? <ScheduleEditor key={`${operations.selectedPlaceId}-${operations.operating.checkedAt}`} regularHours={operations.operating.regularHours} exceptions={operations.operating.operatingExceptions} isSaving={isBusy} onSave={operations.updateOperatingSchedule} /> : <S.PanelBody><S.Empty>영업시간 정보를 불러오지 못했습니다.</S.Empty></S.PanelBody>}</S.Panel></S.Column></S.Workspace>}
