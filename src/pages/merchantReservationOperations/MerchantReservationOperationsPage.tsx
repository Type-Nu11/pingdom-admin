...
function MerchantReservationOperationsPage() {
  ...
  return (
    <Store.Page>
      ...
      {cancelTarget ? <S.ModalOverlay role="presentation" onMouseDown={() => setCancelTarget(null)}><S.Modal role="dialog" aria-modal="true" aria-labelledby="reservation-cancel-title" onMouseDown={(event) => event.stopPropagation()}><S.ModalHeader><div><S.ModalTitle id="reservation-cancel-title">예약 취소</S.ModalTitle></div><S.CloseButton type="button" aria-label="닫기" onClick={() => setCancelTarget(null)}>close</S.CloseButton></S.ModalHeader><S.ModalBody><S.ReadonlyNotice>예약 #{cancelTarget.id}을 취소합니다. 취소하면 예약 가능 인원이 복구됩니다.</S.ReadonlyNotice><S.FormActions><S.ActionButton type="button" disabled={operations.activeAction !== null} onClick={() => setCancelTarget(null)}>닫기</S.ActionButton><S.ActionButton type="button" disabled={operations.activeAction !== null} $variant="danger" onClick={() => void cancelReservation()}>{operations.activeAction ? '처리 중' : '예약 취소'}</S.ActionButton></S.FormActions></S.ModalBody></S.Modal></S.ModalOverlay> : null}
    </Store.Page>
  )
}
