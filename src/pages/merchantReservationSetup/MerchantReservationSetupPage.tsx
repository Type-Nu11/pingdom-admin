  ...
  return (
    <Store.Page>
      ...
      <Store.Content>
        ...
        {reservation.profile && reservation.profile.placeIds.length > 1 ? <Store.PlaceSelect disabled={isBusy} aria-label="예약을 관리할 장소 선택" value={reservation.selectedPlaceId ?? ''} onChange={(event) => selectPlace(Number(event.target.value))}>{reservation.profile.placeIds.map((placeId) => <option key={placeId} value={placeId}>연결 장소 #{placeId}</option>)}</Store.PlaceSelect> : null}
