  ...
  return (
    <Shell.AppShell>
      <Shell.SideNav aria-label="관리자 메뉴">
        ...
        <Shell.SideMenu><Shell.MenuButton type="button" $active onClick={() => navigate('/dashboard')}><Shell.MaterialIcon aria-hidden="true">dashboard</Shell.MaterialIcon><span>대시보드</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/places')}><Shell.MaterialIcon aria-hidden="true">location_on</Shell.MaterialIcon><span>장소 관리</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/main')}><Shell.MaterialIcon aria-hidden="true">description</Shell.MaterialIcon><span>게시글 관리</span></Shell.MenuButton><Shell.MenuButton type="button" onClick={() => navigate('/bans')}><Shell.MaterialIcon aria-hidden="true">block</Shell.MaterialIcon><span>사용자 밴</span></Shell.MenuButton></Shell.SideMenu>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}>
        ...
        <Shared.Content><Shared.PageStack>
          ...
          <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>심사 상태 필터</Shared.PanelTitle><Shared.PanelDescription>신원과 사업자 상태를 조합해 조회합니다.</Shared.PanelDescription></div><Shared.HeaderActions><S.Select aria-label="신원 검증 상태" value={hook.identityStatus} onChange={(event) => changeFilters(event.target.value as MerchantVerificationStatus | '', hook.businessStatus)}><option value="">신원 전체</option>{Object.entries(LABELS).map(([value,label]) => <option key={value} value={value}>{`신원 ${label}`}</option>)}</S.Select><S.Select aria-label="사업자 검증 상태" value={hook.businessStatus} onChange={(event) => changeFilters(hook.identityStatus, event.target.value as MerchantVerificationStatus | '')}><option value="">사업자 전체</option>{Object.entries(LABELS).map(([value,label]) => <option key={value} value={value}>{`사업자 ${label}`}</option>)}</S.Select></Shared.HeaderActions></Shared.PanelHeader></Shared.Panel>
          {hook.errorMessage ? <Shared.Notice $variant="error">{hook.errorMessage}</Shared.Notice> : null}
          <Shared.Workspace>
            <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>검증 신청</Shared.PanelTitle><Shared.PanelDescription>신청자를 선택해 전체 사업자 번호를 확인합니다.</Shared.PanelDescription></div><Shared.PanelCount>{hook.totalCount.toLocaleString()}건</Shared.PanelCount></Shared.PanelHeader><Shared.ScrollArea>{hook.isLoading && hook.items.length === 0 ? <Shared.EmptyState><strong>검증 신청을 불러오는 중입니다.</strong></Shared.EmptyState> : hook.items.length === 0 ? <Shared.EmptyState><strong>조건에 맞는 신청이 없습니다.</strong></Shared.EmptyState> : <S.CardList>{hook.items.map((item) => <S.RecordButton key={item.userId} type="button" $selected={selectedUserId === item.userId} onClick={() => { setSelectedUserId(item.userId); void hook.fetchDetail(item.userId) }}><S.RecordHeader><S.RecordTitle>{item.businessName}</S.RecordTitle><S.StatusBadge $tone={item.identityStatus === 'APPROVED' && item.businessStatus === 'APPROVED' ? 'success' : item.identityStatus === 'REJECTED' || item.businessStatus === 'REJECTED' ? 'danger' : 'warning'}>{LABELS[item.identityStatus]} / {LABELS[item.businessStatus]}</S.StatusBadge></S.RecordHeader><S.RecordMeta>{item.legalName} · 사용자 #{item.userId}</S.RecordMeta><S.RecordDescription>사업자 번호 {item.maskedBusinessRegistrationNumber}</S.RecordDescription></S.RecordButton>)}</S.CardList>}</Shared.ScrollArea><S.Pagination><Shared.SecondaryButton type="button" disabled={hook.page <= 1 || hook.isLoading} onClick={() => { setSelectedUserId(null); hook.clearDetail(); void hook.fetchItems(hook.identityStatus, hook.businessStatus, hook.page - 1) }}>이전</Shared.SecondaryButton><span>{Math.max(hook.page,1)} / {Math.max(hook.totalPages,1)}</span><Shared.SecondaryButton type="button" disabled={!hook.hasNext || hook.isLoading} onClick={() => { setSelectedUserId(null); hook.clearDetail(); void hook.fetchItems(hook.identityStatus, hook.businessStatus, hook.page + 1) }}>다음</Shared.SecondaryButton></S.Pagination></Shared.Panel>
            <Shared.Panel><Shared.PanelHeader><div><Shared.PanelTitle>검증 신청 상세</Shared.PanelTitle><Shared.PanelDescription>민감한 사업자 번호는 심사 화면에서만 확인합니다.</Shared.PanelDescription></div></Shared.PanelHeader><Shared.CompareBody>{!selectedUserId ? <Shared.EmptyState><strong>심사할 신청을 선택해주세요.</strong></Shared.EmptyState> : hook.isDetailLoading ? <Shared.EmptyState><strong>신청 상세를 불러오는 중입니다.</strong></Shared.EmptyState> : hook.detailErrorMessage ? <Shared.EmptyState><strong>{hook.detailErrorMessage}</strong><Shared.SecondaryButton type="button" onClick={() => void hook.fetchDetail(selectedUserId)}>다시 시도</Shared.SecondaryButton></Shared.EmptyState> : hook.detail ? <><S.RecordHeader><div><S.RecordTitle>{hook.detail.businessName}</S.RecordTitle><S.RecordMeta>{hook.detail.legalName} · 사용자 #{hook.detail.userId}</S.RecordMeta></div></S.RecordHeader><S.DetailGrid><S.DetailItem><dt>법적 이름</dt><dd>{hook.detail.legalName}</dd></S.DetailItem><S.DetailItem><dt>사업자명</dt><dd>{hook.detail.businessName}</dd></S.DetailItem><S.DetailItem><dt>사업자 등록번호</dt><dd>{hook.detail.businessRegistrationNumber}</dd></S.DetailItem><S.DetailItem><dt>신원 상태</dt><dd>{LABELS[hook.detail.identityStatus]}</dd></S.DetailItem><S.DetailItem><dt>사업자 상태</dt><dd>{LABELS[hook.detail.businessStatus]}</dd></S.DetailItem><S.DetailItem><dt>접수 시각</dt><dd>{formatDate(hook.detail.createdAt)}</dd></S.DetailItem><S.DetailItem><dt>심사 관리자</dt><dd>{hook.detail.reviewedBy ? `ID ${hook.detail.reviewedBy}` : '미심사'}</dd></S.DetailItem><S.DetailItem><dt>심사 사유</dt><dd>{hook.detail.reviewReason || '아직 입력되지 않음'}</dd></S.DetailItem></S.DetailGrid><Shared.DetailNotice><Shell.MaterialIcon aria-hidden="true">info</Shell.MaterialIcon><div><strong>현재 API가 제공하는 증빙 범위</strong>법적 이름과 사업자 등록번호 외 별도 문서 URL은 응답 계약에 포함되지 않습니다.</div></Shared.DetailNotice><S.InlineActions><Shared.PrimaryButton type="button" disabled={hook.isReviewing} onClick={openReview}>심사 결과 입력</Shared.PrimaryButton></S.InlineActions></> : null}</Shared.CompareBody></Shared.Panel>
          </Shared.Workspace>
