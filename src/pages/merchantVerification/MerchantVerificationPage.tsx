  ...
  return (
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}>
        <Shell.TopBar><Shell.TopTitleGroup><Shell.TopTitle>상점주 검증 심사</Shell.TopTitle></Shell.TopTitleGroup><Shell.TopActions><AdminNotificationButton /><Shell.IconButton type="button" aria-label="목록 새로고침" disabled={hook.isLoading || hook.isReviewing} onClick={() => void hook.fetchItems(hook.identityStatus, hook.businessStatus, hook.page)}><Shell.MaterialIcon aria-hidden="true">refresh</Shell.MaterialIcon></Shell.IconButton></Shell.TopActions></Shell.TopBar>
        <Shared.Content><Shared.PageStack>
          <Shared.PageHeader><div><Shared.Eyebrow>검토함 &gt; 상점주 본인·사업자 검증</Shared.Eyebrow><Shared.PageTitle>상점주 본인·사업자 검증</Shared.PageTitle><Shared.PageDescription>신청자의 법적 이름과 사업자 등록 정보를 각각 판정합니다.</Shared.PageDescription></div><Shared.HeaderActions><Shared.HeaderButton type="button" onClick={() => navigate('/merchant-owners')}>상점주 신청</Shared.HeaderButton></Shared.HeaderActions></Shared.PageHeader>
