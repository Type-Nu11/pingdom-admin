...
function MerchantVerifiedBoostPage() {
  ...
  if (isInitialFailure) {
    return <Store.Page><Store.Header><Store.BrandLogo src="/pingdom-logo.png" alt="PingDom" /><Store.LogoutButton type="button" onClick={handleLogout}>로그아웃</Store.LogoutButton></Store.Header><Store.Content><Store.PageIntro><div><Store.PageTitle>Verified Boost 관리</Store.PageTitle></div></Store.PageIntro><Store.Notice $tone="error" role="alert"><Store.NoticeIcon aria-hidden="true">error_outline</Store.NoticeIcon>Verified Boost 정보를 불러오지 못했습니다.</Store.Notice><div style={{ marginTop: 16 }}><Store.RetryButton type="button" onClick={() => { void boost.fetchSelections(1, true); void boost.fetchExecutions(1, true); void boost.fetchProducts(true); void boost.fetchProfile(true) }}>다시 시도</Store.RetryButton></div></Store.Content></Store.Page>
  }
