...
const ROLES: Record<AdminRole, string> = {
  SUPER_ADMIN: "최고 관리자",
  CONTENT_MODERATOR: "콘텐츠 운영자",
  MERCHANT_OPERATOR: "Merchant 운영자",
  SUPPORT_OPERATOR: "사용자 지원 운영자",
  ANALYST: "분석 담당자",
};
const PERMISSIONS: Record<string, string> = {
  ADMIN_ROLE_MANAGE: "관리자 역할 관리",
  USER_READ: "사용자 조회",
  USER_SANCTION: "사용자 제재",
  PLACE_READ: "장소 조회",
  PLACE_MODERATE: "장소 운영",
  MERCHANT_REVIEW: "상점주 심사",
  REPORT_REVIEW: "신고 심사",
  SCOUT_REVIEW: "탐색 후보 심사",
  DASHBOARD_READ: "대시보드 조회",
  AUDIT_READ: "감사 로그 조회",
  OUTBOX_RECOVERY: "발송 재처리",
};
...
function UserRolePage() {
  ...
  const submit = async () => {
    if (!dialog || hook.isMutating) return;
    if (!reason.trim()) {
      setFormError("권한 변경 사유를 입력해주세요.");
      return;
    }
    if (await hook.mutate(dialog.role, dialog.action, reason.trim()))
      setDialog(null);
  };
  ...
  return (
    <Shell.AppShell>
      ...
      <Shell.MainArea id={ADMIN_MAIN_SCROLL_AREA_ID}>
        <Shell.TopBar>
          <Shell.TopTitleGroup>
            <Shell.TopTitle>관리자 역할 관리</Shell.TopTitle>
          </Shell.TopTitleGroup>
          <Shell.TopActions>
        ...
        <Shared.Content>
          <Shared.PageStack>
            <Shared.PageHeader>
              <div>
                <Shared.Eyebrow>사용자 관리 &gt; 관리자 역할</Shared.Eyebrow>
                <Shared.PageTitle>관리자 역할 및 권한 이력</Shared.PageTitle>
                <Shared.PageDescription>
                  관리자 사용자 ID를 조회하고 활성 역할과 변경 이력을 확인한 뒤
                  권한을 부여하거나 회수합니다.
                </Shared.PageDescription>
              </div>
            ...
            {hook.targetUserId && user?.id === hook.targetUserId ? (
              <Shared.DetailNotice>
                <Shell.MaterialIcon aria-hidden="true">
                  warning
                </Shell.MaterialIcon>
                <div>
                  <strong>현재 로그인한 계정입니다.</strong>자기 역할을 회수하면
                  관리 기능에 다시 접근하지 못할 수 있습니다.
                </div>
              </Shared.DetailNotice>
            ) : null}
            <Shared.Panel>
              <Shared.PanelHeader>
                ...
                <Shared.PanelCount>{activeRoles.size}개 활성</Shared.PanelCount>
              </Shared.PanelHeader>
              <Shared.CompareBody>
                {!hook.targetUserId ? (
                  <Shared.EmptyState>
                    <strong>관리자 사용자를 조회해주세요.</strong>
                  </Shared.EmptyState>
                ) : (
                  <S.CardList>
                    {(Object.entries(ROLES) as [AdminRole, string][]).map(
                      ([role, label]) => {
                        const active = activeRoles.has(role);
                        return (
                          <S.RecordCard key={role}>
                            <S.RecordHeader>
                              <div>
                                <S.RecordTitle>{label}</S.RecordTitle>
                                <S.RecordMeta>{role}</S.RecordMeta>
                              </div>
                              <S.StatusBadge
            ...
            <Shared.Panel>
              <Shared.PanelHeader>
                <div>
                  <Shared.PanelTitle>역할 변경 이력</Shared.PanelTitle>
                  <Shared.PanelDescription>
                    최근 할당 순서로 부여자와 권한 범위를 표시합니다.
                  </Shared.PanelDescription>
                </div>
                <Shared.PanelCount>
                  {hook.assignments.length}건
                </Shared.PanelCount>
              </Shared.PanelHeader>
              <Shared.CompareBody>
                {hook.targetUserId && hook.assignments.length === 0 ? (
                  <Shared.EmptyState>
                    <strong>역할 변경 이력이 없습니다.</strong>
                  </Shared.EmptyState>
                ) : (
                  <S.CardList>
                    {hook.assignments.map((item) => (
                      <S.RecordCard key={item.id}>
                        <S.RecordHeader>
                          <div>
                            <S.RecordTitle>{ROLES[item.role]}</S.RecordTitle>
                            <S.RecordMeta>
                              이력 #{item.id} · 처리 관리자 #
                          ...
                          >
                            {item.status === "ACTIVE" ? "활성" : "회수"}
                          </S.StatusBadge>
                        </S.RecordHeader>
                        <S.DetailGrid>
                          <S.DetailItem>
                            <dt>부여 시각</dt>
                            <dd>{date(item.assignedAt)}</dd>
                          </S.DetailItem>
                          <S.DetailItem>
                            <dt>회수 시각</dt>
                            <dd>{date(item.revokedAt)}</dd>
                          </S.DetailItem>
                        </S.DetailGrid>
                        <S.RecordDescription>
                          {item.permissions
      ...
      {dialog ? (
        <Shared.ModalOverlay
          role="presentation"
          onMouseDown={() => !hook.isMutating && setDialog(null)}
        >
          <Shared.Modal
            role="dialog"
            aria-modal="true"
          ...
          >
            <Shared.ModalHeader>
              <Shared.ModalTitle id="role-dialog-title">
                관리자 역할 {dialog.action === "assign" ? "부여" : "회수"}
              </Shared.ModalTitle>
            ...
            <Shared.ModalBody>
              <Shared.ModalWarning>
                관리자 #{hook.targetUserId}의 {ROLES[dialog.role]} 역할을{" "}
                {dialog.action === "assign" ? "부여" : "회수"}합니다. 권한
                범위가 즉시 변경될 수 있습니다.
              </Shared.ModalWarning>
              <S.Section>
                <S.Field>
