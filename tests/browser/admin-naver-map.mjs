...
try {
  ...
  for (const width of [1280, 390]) {
    ...
    await page.waitForFunction(() => window.naverTest.stats.maps[0].getZoom() === 21)
    await page.getByRole('button', { name: '지도 축소' }).click()
    await page.waitForFunction(() => window.naverTest.stats.maps[0].getZoom() === 20)
    ...
    await page.waitForFunction(() => document.querySelectorAll('.pingdom-map-marker').length === 1)
    const beforeRefresh = await page.evaluate(() => ({ maps: window.naverTest.stats.maps.length, destroyed: window.naverTest.stats.destroyed, fits: window.naverTest.stats.centers.length }))
    await page.getByRole('button', { name: '장소 목록 새로고침', exact: true }).click()
    await page.waitForFunction(count => window.naverTest.stats.centers.length > count, beforeRefresh.fits)
    assert.equal(await page.evaluate(() => window.naverTest.stats.maps.length), beforeRefresh.maps)
    assert.equal(await page.evaluate(() => window.naverTest.stats.destroyed), beforeRefresh.destroyed)
    await page.waitForFunction(() => document.querySelectorAll('.pingdom-map-marker').length === 1)
