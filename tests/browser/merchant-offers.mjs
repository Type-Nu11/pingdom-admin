...
try {
  ...
  for (const width of [1280, 390]) {
    ...
    assert.equal(await page.evaluate(() => window.qaRequests.filter(r => r.url === '/merchant-owner/offers').length), 1)
    const next = page.getByRole('button', { name: '다음 페이지로 이동' })
    await next.focus()
    ...
    assert.equal(await title.inputValue(), '작성 중인 새 혜택')
    await page.keyboard.press('Enter')
    await page.getByText('합성 혜택 41', { exact: true }).waitFor()
    assert.equal(await page.locator('button[aria-current="page"]').evaluate(el => el === document.activeElement), true)
    await page.screenshot({ path: join(output, `pagination-focus-${width}.png`) })
    // Loading keeps pagination mounted; moving focus into the draft must not be undone.
    await page.evaluate(() => { window.qaHoldList = true })
    await page.getByRole('button', { name: '이전 페이지로 이동' }).click()
    await page.waitForFunction(() => typeof window.qaReleaseList === 'function')
    assert.equal(await next.count(), 1)
    assert.equal(await next.isDisabled(), true)
    await title.focus()
    await page.evaluate(() => { window.qaHoldList = false; window.qaReleaseList(); window.qaReleaseList = null })
    await page.getByText('합성 혜택 21', { exact: true }).waitFor()
    assert.equal(await title.evaluate(el => el === document.activeElement), true)
    // Failed page loads keep navigation usable without exposing the previous rows.
    await page.evaluate(() => { window.qaFailNextList = true })
    await next.focus()
    await page.keyboard.press('Enter')
    await page.getByRole('button', { name: '목록 다시 조회' }).waitFor()
    assert.equal(await next.count(), 1)
    assert.equal(await page.getByText('합성 혜택 21', { exact: true }).count(), 0)
    assert.equal(await page.locator('button[aria-current="page"]').evaluate(el => el === document.activeElement), true)
    ...
    assert.equal(await page.locator('button[aria-current="page"]').textContent(), '3')
    ...
    await page.getByRole('combobox', { name: '혜택을 관리할 장소 선택' }).click()
