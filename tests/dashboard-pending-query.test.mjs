...
const pending={items:[{type:'MERCHANT_PLACE_APPLICATION',targetId:7,status:'PENDING',title:'합성 가게'}],totalCount:30}
async function finish(start=0,data=pending,errorStatus){
  await act(async()=>{
    for(const request of requests.slice(start,start+3)){
      ...
      if(config.url.endsWith('/pending-items') && errorStatus){reject(new AxiosError('Synthetic failure','ERR_BAD_RESPONSE',config,null,{config,status:errorStatus,statusText:'Error',headers:{},data:{}}));continue}
      resolve({config,status:200,statusText:'OK',headers:{},data:config.url.endsWith('/pending-items')?data:config.url.endsWith('/summary')?{placeCount:1,bannedUserCount:0}:{places:[],userSanctions:[]}})
    }
  })
}
test('uses unified GET once with limit 10 and blocks overlapping refresh',async()=>{
  assert.equal(requests.length,3)
  assert.equal(requests[2].config.url,'/admin/dashboard/pending-items')
  assert.deepEqual(requests[2].config.params,{limit:10})
...
test('refresh failure retains previous result and retry can reach a genuine zero',async()=>{
  ...
  await act(async()=>{void state.fetchSummary()});await finish(6,{items:[],totalCount:0})
  assert.equal(state.pendingItemsStatus,'empty');assert.equal(state.pendingItems.totalCount,0)
})
for(const data of [{items:null,totalCount:0},{items:[null],totalCount:1},{items:[],totalCount:-1}]) test(`malformed response ${JSON.stringify(data)} is not zero`,async()=>{
...
test('disabled session invalidates old response without clearing a newer request',async()=>{
  ...
  await finish(0,{items:[],totalCount:0})
